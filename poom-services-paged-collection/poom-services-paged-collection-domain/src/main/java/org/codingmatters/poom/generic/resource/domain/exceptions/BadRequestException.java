...
public class BadRequestException extends Exception {
    ...
    public BadRequestException(CategorizedLogger log, String message) {
        this(Error.builder().code(Error.Code.BAD_REQUEST).token(log.tokenized().error(message)).description(message).build(), message);
    }

    public BadRequestException(CategorizedLogger log, String message, Throwable cause) {
        this(Error.builder().code(Error.Code.BAD_REQUEST).token(log.tokenized().error(message, cause)).description(message).build(), message, cause);
    }
